...
from iirosebot.API.api_iirose import APIIirose, PlatformType
...
async def playlist_offset(Message, music_list_name, offset, search_type="1000"):
    ...
    request = requests.get(
        f'https://xc.null.red:8043/api/netease/cloudsearch?keywords={music_list_name}&limit=10&type={search_type}{m_offset}').json()
...
async def play_n_media(n_music_id, name_text=None):
    ...
    if name_text != "[电台":
        ...
        song_info = requests.get(f'https://xc.null.red:8043/api/netease/song/detail?ids={n_music_id}').json()['songs'][
            0]
        ...
        song_info = requests.get(f'https://xc.null.red:8043/api/netease/dj/program/detail?id={n_music_id}').json()[
            'program']
...
@on_command(f'{com_head}点歌 ', True, command_type=[MessageType.room_chat, MessageType.private_chat])
...
@on_command(f'{com_head}点歌id ', True, command_type=[MessageType.room_chat, MessageType.private_chat])
...
@on_command(f'{com_head}列表 ', True, command_type=[MessageType.room_chat, MessageType.private_chat])
...
@on_command(f'{com_head}歌单 ', True, command_type=[MessageType.room_chat, MessageType.private_chat])
async def music_list(Message, text):
    ...
    if list_text[0] == '搜索':
        ...
        request = requests.get(
            f'https://xc.null.red:8043/api/netease/cloudsearch?keywords={music_list_name}&limit=10&type=1000').json()
        ...
        async def play_n_playlist(list_id):
            list_date = requests.get(f"https://xc.null.red:8043/api/netease/playlist/detail?id={list_id}").json()
            if list_date['code'] != 200:
                return 400
            if list_date['playlist']['trackCount'] > 1000:
                re_num = int(list_date['playlist']['trackCount'] / 1000) + 1
                for i in range(re_num):
                    i += 1
                    request = requests.get(
                        f'https://xc.null.red:8043/api/netease/playlist/track/all?id={list_id}&limit={i * 1000}&offset={i * 1000 - 1000}').json()
                    if request['code'] == 404:
                        return 404

                    list_data = f"{list_date['playlist']['name']} by: {list_date['playlist']['creator']['nickname']}"

                    for song_data in request['songs']:
                        auther = ''
                        for ar in song_data['ar']:
                            auther += ar['name'] + '/'
                        play_list_song.append([song_data["id"], song_data['name'], auther[:-1], '歌单', list_data])
            else:
...
@on_command(f'{com_head}专辑 ', True, command_type=[MessageType.room_chat, MessageType.private_chat])
async def album_list(Message, text):
    ...
    if list_text[0] == '搜索':
        music_list_name = text[3:]
        request = requests.get(
            f'https://xc.null.red:8043/api/netease/cloudsearch?keywords={music_list_name}&limit=10&type=10').json()
...
@on_command(f'{com_head}电台 ', True, command_type=[MessageType.room_chat, MessageType.private_chat])
async def radio_list(Message, text):
    ...
    if list_text[0] == '搜索':
        music_list_name = text[3:]
        request = requests.get(
            f'https://xc.null.red:8043/api/netease/cloudsearch?keywords={music_list_name}&limit=10&type=1009').json()
        ...
        if request['code'] == 200:
            ...
            for playlist_data in request['result']['djRadios']:
                ...
                list_id = requests.get(
                    "https://xc.null.red:8043/api/netease/dj/detail?rid=" + playlist_data['id']).json()
...
@on_command(f'{com_head}歌手 ', True, command_type=[MessageType.room_chat, MessageType.private_chat])
async def radio_list(Message, text):
    global play_list_song
    ...
    if list_text[0] == '搜索':
        music_list_name = text[3:]
        request = requests.get(
            f'https://xc.null.red:8043/api/netease/cloudsearch?keywords={music_list_name}&limit=10&type=100').json()
...
@on_command(f'{com_head}模式 ', True, command_type=[MessageType.room_chat, MessageType.private_chat])
...
async def room_message(Message):
    ...
    if Message.user_id in wait_user:
        ...
        if wait_user[Message.user_id][-1] == 'music':
            ...
            list_date = requests.get(f"https://xc.null.red:8043/api/netease/playlist/detail?id={playlist}").json()
            if list_date['code'] != 200:
                return 400
            if list_date['playlist']['trackCount'] > 1000:
                re_num = int(list_date['playlist']['trackCount'] / 1000) + 1
                for i in range(re_num):
                    i += 1
                    request = requests.get(
                        f'https://xc.null.red:8043/api/netease/playlist/track/all?id={playlist}&limit={i * 1000}&offset={i * 1000 - 1000}').json()
                    if request['code'] == 404:
                        return 404
...
def play_playlist_song():
    ...
    while True:
        try:
            ...
            if play_list_song:
                random.seed(int(str(random.SystemRandom().random())[2:]))
                play_playlist = True
                random.shuffle(play_list_song)
